...
import numpy as np
import setup_irm_moment_matching as setup
import datetime
...
reg_moments, reg_var, lr, gamma_moments, moments_epoch, sinkhorn_iter, filename = float(sys.argv[1]), float(sys.argv[2]), float(sys.argv[3]), float(sys.argv[4]), int(float(sys.argv[5])), int(float(sys.argv[6])), sys.argv[7]
num_steps = 15000
#sinkhorn_iter = 5
fitted_graph, current_time, expt_id = setup.IRM(data_train, data_test, num_steps=num_steps, 
                        reg_moments=reg_moments, reg_var = reg_var, learning_rate = lr, 
                        moments_epoch = moments_epoch, gamma_moments = gamma_moments, sinkhorn_iter = sinkhorn_iter, moments_start_step= 400)



accuracy = {'reg_moments': reg_moments, 'reg_var': reg_var, 'learning_rate': lr, 'datetime': current_time, 
            'num-steps': num_steps, 'moments-epoch': moments_epoch, 'sinkhorn-iteration': sinkhorn_iter}
accuracy['id'] = expt_id
