...
def IRM(data_train, data_test, batch_size = 1500, num_steps = 2500, 
                    learning_rate = 1e-5, 
                    reg_moments = 0, moments_epoch = 1, 
                    gamma_moments = 1e-2, moments_start_step = 50,
    ...
    Input: 
        ...
        reg_moments: (float) regularization parameter for moments regularizer
        moments_epoch: (int) epoch interval at which wasserstin regularizer is activated
        gamma_moments: (float) gamma parameter in sinkhorn algorithm
        moments_start_step: (int) epoch at which moments regularized is activated for the first time
    ...
    train_moments_y = dict()
    for y in [0,1]:
        train_moments_y[y] = tf.keras.metrics.Mean('train_moments_y:'+str(y), dtype=tf.float32)
    ...
    parameter = f'_expt-id_{expt_id}_num_steps_{num_steps}_lr_{learning_rate}_reg_moments_{reg_moments}_moments_epoch_{moments_epoch}_reg_var_{reg_var}_gamma_moments_{gamma_moments}_sinkhorn_iter_{sinkhorn_iter}'
    ...
    def train_step(data_train_epoch, data_train_moments, full_data, step):
        ...
        with tf.GradientTape() as g:
            ...
            loss_train = loss
            WD = [0,0]
            if step % moments_epoch == 0:
                for label in [0,1]:
                    conditional_data = [env[0][env[1][:, 1] == label] for env in data_train_moments]
                    # normal sinkhorn: sh.sinkhorn_dist
                    # symmetric sinkhorn: sh.sys_sinkhorn
                    phi0, phi1 = graph.invariant_map(conditional_data[0]), graph.invariant_map(conditional_data[1])
                    moments_dist = loss = tf.cast(0, dtype = tf.float32)
                    for i in [3, 4, 5]:
                        moments_dist += (utils.moment(phi0, order=i) - utils.moment(phi1, order=i))**2
                    
                    train_moments_y[label](moments_dist)
                    WD[label] = moments_dist
                    if step > moments_start_step:
                        loss = loss + reg_moments*moments_dist
    ...
    for step, data in enumerate(zip(*batch_data), 1):
        ...
        #_, _ = train_step(batch_data_train, data_train, data_train, step) # If using full data to calculate moments distance
        _, _, train_probs = train_step(batch_data_train, batch_data_train, data_train, step) # If using batch data to calculate moments distance
        ...
        with train_summary_writer.as_default():
            ...
            for y in [0,1]:
                tf.summary.scalar('moments-train-y:'+str(y), train_moments_y[y].result(), step = step)  
        ...
        if step % moments_epoch == 0:    
            for y in [0,1]:
                train_moments_y[y].reset_states()
