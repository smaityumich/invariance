...
def dist(a, b, normalize=False):
    ...
    M = 0.5 * tf.reduce_sum(tf.square(tf.subtract(M1, M2)), axis=2)  # (na, nb)
    return M
...
def sinkhorn_log_stabilized(a, b, M, reg, L, tau = 1e3):
    ...
    for _ in range(L):
        ...
        if tf.math.reduce_max(tf.math.abs(u)) > tau or tf.math.reduce_max(tf.math.abs(v)) > tau:
            ...
            v, u = tf.ones([b.shape[0],], dtype = tf.float32), \
                tf.ones([a.shape[0],], dtype = tf.float32)
...
def sinkhorn_dist(x, y, reg, L, normalize=False):
    M = tf.square(dist(x, y, normalize))
...
def sym_sinkhorn(x, y, reg, L, normalize=False):
    return sinkhorn_dist(x, y, reg, L, normalize) - 0.5 * (sinkhorn_dist(x, x, reg, L, normalize) + sinkhorn_dist(y, y, reg, L, normalize))
