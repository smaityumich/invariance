...
class InvarianceNNGraph(keras.Model, keras.layers.Layer):
    ...
    def __init__(self):
        ...
        self.weight = {'weight1': self.add_weight(shape=(392, 50), initializer=initializer, trainable=True), 
                        'weight2': self.add_weight(shape=(50, 50), initializer=initializer, trainable=True), 
                        'weight3': self.add_weight(shape=(50, 1), initializer=initializer, trainable=True)}
                        
        self.bias = {'bias1': self.add_weight(shape=(50, ), initializer=initializer, trainable=True), 
