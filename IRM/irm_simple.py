...
class InvarianceNNGraph(keras.Model, keras.layers.Layer):
    ...
    def __init__(self):
        ...
        self.weight = {'weight1': self.add_weight(shape=(392, 10), initializer=initializer, trainable=True), 
                        'weight2': self.add_weight(shape=(10, 5), initializer=initializer, trainable=True), 
                        'weight3': self.add_weight(shape=(5, 1), initializer=initializer, trainable=True)}
                        
        self.bias = {'bias1': self.add_weight(shape=(10, ), initializer=initializer, trainable=True), 
